...
static uint8_t fd_BMP = 0x77;
// start of the eeprom
static uint8_t eeprom_start = 0xAA;

// value from temperature needed for pressure conversion
static int32_t b5 = 0xFFFFFFFF;
...
*/
void al_bmp180_get_calib_param(i2c_master_bus_handle_t bus_handle,
                               uint8_t slave_addr,
                               uint8_t eeprom_start)
{
    ESP_LOGI(TAG, "Started getting calibration parameter");

    calib_param.ac1 = get_int_param(bus_handle, slave_addr, eeprom_start);
    calib_param.ac2 = get_int_param(bus_handle, slave_addr, eeprom_start + 2);
    calib_param.ac3 = get_int_param(bus_handle, slave_addr, eeprom_start + 4);
    calib_param.ac4 = get_uint_param(bus_handle, slave_addr, eeprom_start + 6);
    calib_param.ac5 = get_uint_param(bus_handle, slave_addr, eeprom_start + 8);
    calib_param.ac6 = get_uint_param(bus_handle, slave_addr, eeprom_start + 10);
    calib_param.b1 = get_int_param(bus_handle, slave_addr, eeprom_start + 12);
    calib_param.b2 = get_int_param(bus_handle, slave_addr, eeprom_start + 14);
    calib_param.mb = get_int_param(bus_handle, slave_addr, eeprom_start + 16);
    calib_param.mc = get_int_param(bus_handle, slave_addr, eeprom_start + 18);
    calib_param.md = get_int_param(bus_handle, slave_addr, eeprom_start + 20);

    ESP_LOGI(TAG, "Finshed getting calibration parameters");
}

esp_err_t al_bmp180_init(i2c_master_bus_handle_t bus_handle)
{
    ...
    if (0x55 != read_byte(bus_handle, fd_BMP, 0xD0))
    {
    ...
    al_bmp180_get_calib_param(bus_handle, fd_BMP, eeprom_start);
...
**Description**
    ...
    console.
*/
int32_t al_bmp180_get_ut(i2c_master_bus_handle_t bus_handle,
                         uint8_t slave_addr)
{
    int32_t ut = 0;
    ...
    // 0xF4
    write_byte(bus_handle, slave_addr, 0xF4, 0x2E);

    // delay time: 4.5ms = 4500µs
    vTaskDelay(5);

    // read out uncompensated temperature
    ut = ((int32_t)read_byte(bus_handle, slave_addr, 0xF6) << 8);
    ut += (int32_t)read_byte(bus_handle, slave_addr, 0xF7);
...
*/
int32_t al_bmp180_get_up(i2c_master_bus_handle_t bus_handle,
                         uint8_t slave_addr,
                         uint8_t oss)
{
    int32_t msb = 0;
    ...
    {
    case 0:
        write_byte(bus_handle, slave_addr, 0xF4, 0x34);
        // conversion time: 4.5ms = 4500µs
        vTaskDelay(5);
        break;

    case 1:
        write_byte(bus_handle, slave_addr, 0xF4, 0x74);
        // conversion time: 7.5ms = 7500µs
        vTaskDelay(8);
        break;

    case 2:
        write_byte(bus_handle, slave_addr, 0xF4, 0xB4);
        // Conversion time: 13.5ms = 13500µs
        vTaskDelay(14);
        break;

    case 3:
        write_byte(bus_handle, slave_addr, 0xF4, 0xF4);
    ...
    // read measured data
    msb = read_byte(bus_handle, slave_addr, 0xF6);
    lsb = read_byte(bus_handle, slave_addr, 0xF7);
    xlsb = read_byte(bus_handle, slave_addr, 0xF8);
    up = msb;
    up <<= 8;
    up += lsb;
    up <<= 8;
...
float al_bmp180_get_temperature(i2c_master_bus_handle_t bus_handle)
{
    int32_t ut = al_bmp180_get_ut(bus_handle, fd_BMP);
    int32_t t, x1, x2;
    ...
    ESP_LOGD(TAG,
             "temperature in degrees Celsius: %.1f",
             (float)t / 10);

    return (float)t / 10;
}

float al_bmp180_get_pressure(i2c_master_bus_handle_t bus_handle, uint8_t oss)
{
    int32_t up = 0;
    ...
    // get the uncompensated pressure
    up = al_bmp180_get_up(bus_handle, fd_BMP, oss);
